...
function buildHeaders(request) {
    return buildHeadersAuthFirstCustomLast(request)(addEtagHeader(request.config), function (headers) { return csrf_token_header_1.addCsrfTokenAndCookies(request, headers); }, addAcceptHeader, addContentTypeHeader, addSapClientHeader(request.destination.sapClient), exports.addProxyHeaders(request.destination.proxyConfiguration), addLocationIdHeader(request.destination.cloudConnectorLocationId));
}
...
exports.buildHeadersForDestination = function (destination) {
    return cloud_sdk_util_1.asyncPipe(addSapClientHeader(destination.sapClient), exports.addProxyHeaders(destination.proxyConfiguration), addLocationIdHeader(destination.cloudConnectorLocationId), authorization_header_1.buildAndAddAuthorizationHeader(destination))({});
};
...
var addLocationIdHeader = function (locationId) { return function (headers) { return cloud_sdk_util_1.assocSome('SAP-Connectivity-SCC-Location_ID', locationId)(headers); }; };
