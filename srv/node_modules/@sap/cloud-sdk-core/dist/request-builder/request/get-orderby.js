...
function getOrderByExpressionForOrderLink(orderBy, parentFieldNames) {
    if (parentFieldNames === void 0) { parentFieldNames = []; }
    return getODataOrderByExpressions(orderBy.orderBy, __spreadArrays(parentFieldNames, [orderBy.link.fieldName])).join(',');
}
function getOrderByExpressionForOrder(orderBy, parentFieldNames) {
    if (parentFieldNames === void 0) { parentFieldNames = []; }
    return [__spreadArrays(parentFieldNames, [orderBy.fieldName]).join('/'), orderBy.orderType].join(' ');
}
